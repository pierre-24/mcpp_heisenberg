...
TEST_F(AppTestsSuite, TestSave) {
  ...
  // read back and check
  {
    ...
    std::vector<double> expected_parameters = {parameters.kB, parameters.T, parameters.muB, parameters.H};
    EXPECT_EQ(xparameters, expected_parameters);
...
class AppQuantumSpinTestsSuite : public MCHTestsSuite {
 protected:
  mch::Geometry geometry;
  mch::app::Parameters parameters;
  std::filesystem::path temp_path;

  AppQuantumSpinTestsSuite() {
    // Create temporary
    temp_path = std::filesystem::temp_directory_path() / std::tmpnam(nullptr);
    ...
    geometry = mch::Geometry("BCC", lattice, {{"B", 1}, {"N", 1}}, positions);

    // Tweak a few parameters
    parameters.T = 5.0;
    parameters.N = 5000;
    parameters.supercell_size = {3, 3, 3};
    parameters.magnetic_sites = {"B", "N"};
    parameters.pair_defs = {
        {"B", "B", 2.0, 1.0},
        {"N", "N", 2.0, 0.1},
        {"B", "N", 2.598, -1.0},
    };

    parameters.use_quantum_spins = true;
    parameters.spin_values["N"] = 1.5;
  }
...
/// Test if H5 contains what it should
TEST_F(AppQuantumSpinTestsSuite, TestSave) {
  parameters.T = 5.0;
  parameters.N = 200;
  parameters.H = 0.5;
  ...
  // read back and check
  {
    ...
    std::vector<std::string> ion_types;
    geometry_group.getDataSet("ion_types").read(ion_types);
    EXPECT_EQ(ion_types, std::vector<std::string>({"B", "N"}));

    std::vector<uint64_t> ion_numbers;
    geometry_group.getDataSet("ion_numbers").read(ion_numbers);
    EXPECT_EQ(ion_numbers, std::vector<uint64_t>({27, 27}));

    std::vector<double> spin_values;
    geometry_group.getDataSet("spin_values").read(spin_values);
    std::vector<double> expected_spin_values(27, 1.0);
    expected_spin_values.insert(expected_spin_values.end(), 27, 1.5);
    EXPECT_EQ(spin_values, expected_spin_values);
    ...
    std::vector<double> magnetic_anisotropies;
    hamiltonian_group.getDataSet("magnetic_anisotropies").read(magnetic_anisotropies);
    EXPECT_EQ(magnetic_anisotropies, std::vector<double>(54, 0.0));
    ...
    arma::Mat<float> xconfigs(54, parameters.N);
    results_group.getDataSet("configs").read_raw(xconfigs.memptr());
    arma::mat configs = arma::conv_to<arma::mat>::from(xconfigs);

    auto hamiltonian = mch::IsingHamiltonian(54, xpairs, magnetic_anisotropies);
